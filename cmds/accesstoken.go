...
func AccessTokenCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "access-token",
		Short: "Get access token for Ripple/Wave(Pro)",
		Long:  `Get access token for Ripple/Wave(Pro). See global flags for more information on the default environment variables.`,
